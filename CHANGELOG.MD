...
- **Updates**
  - Updated to Foundry VTT version `14`
  - Updated how data is packed and unpacked from compendiums (should also fix the scenes with no bg issue)
  - Changed SFX to use `GGG` for the `Break the Seal` animation

## 2.0.3

- Fixed bug where "Update Available" message is sent everytime someone logs in (🐛 @Dio)

## 2.0.2

- **Updated**
  - Fixed Error with loading module (@weepingminotaur)
  - Fixed visual bug causing the Drawings Setup % to show frozen even though it was finished (@elorebaen)
    - IE `Drawings Setup` does **not** need to be rerun if it ended at 88%

## 2.0.1

- Updated Russian Translation (🌐 @Mikkidez)

## 2.0.0

- **New**
  - Module updated to newest Abomination Vaults release and generally requires version `4.0.1` or higher
  - `Murmur, Implement of Empty Death` - A rework of Murmur (🧠 @weepingminotaur)
  - Added new setting to receive a reminder message to import an update
- **Updated**
  - Fixed bug with Floor Helper Drawings setup (🐛 @reins864)
  - Fixed issue with Monk's Active Tile Triggers art of the downhill map (🐛 @reins864)

## 1.6.2

- Added Chinese Translation (🌐 @Mtakaqiao)

## 1.6.1

- Fixed issue with databases not updating (should fix all scene related issues)

## 1.6.0 - Yldaris Settlement

- **New**
  - Added `Yldaris Settlement Treatment` to the main journal

## 1.5.1 - Russian

- **New**
  - Added Russian Translation (🌐 @Mikkidez)

## 1.5.0 - Title Card and more

- **New**
  - `Title Card Animation` - Animation to play the title card with cool sfx and a voice over cameo from an iconic voice actor
  - `Wrin's Wonders Exterior` - Added art for Wrin's Wonders Exterior from (@morlockhater)
- **Updated**
  - Improved landing journal navigation to make understanding this module a bit easier

## 1.4.0 - Creature Craftables

- Added new entry listing all the possible creature craftables

# 1.3.3 - Fix to fix to fix

- Actually fixed `Prison Flames` animation bug
